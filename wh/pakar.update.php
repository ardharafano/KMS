<?php
	...
	$kode = $_GET['kode'];
	$sql	= $conn->query("select * from penyakit_gejala where kode_penyakit='$kode'");
            	...
            	<form class="form-horizontal" name="form1" method="post" action="" enctype="multipart/form-data">
                    <div class="control-group">
                        ...
                        <div class="controls">
                            <select class="input-xxlarge" name="kode_penyakit" id="kode_penyakit">
								<?php
									$sp0 = $conn->query("select * from penyakit where kode_penyakit='$kode'");
									if($dp0 = mysqli_fetch_array($sp0)){
                                		echo "<option value='$kode'>$kode == $dp0[nama_penyakit]</option>";
									}
								?>
                                <?php
                                    $sp = $conn->query("select * from penyakit");
                    ...
                    <?php }
						if(isset($_POST['simpan'])){
							$conn->query("delete from penyakit_gejala where kode_penyakit='$kode'") or die(mysqli_error());
							...
							for($i=0; $i < $jumlah; $i++){
								...
								$q=$conn->query("insert into penyakit_gejala(kode_penyakit, id_gejala) values ('$_POST[kode_penyakit]', '$gejala')") or die(mysqli_error());
							}
