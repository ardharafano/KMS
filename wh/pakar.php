			...
			<div>
                ...
                <div style="margin-top:10px">
                    <table class="table table-condensed table-bordered table-hover">
                        ...
                        <?php
                            $i = 1;
                            $spg = $conn->query("select * from penyakit_gejala group by kode_penyakit desc");
                            while($dpg = mysqli_fetch_array($spg)){
                                $sp = $conn->query("select * from penyakit where kode_penyakit='$dpg[kode_penyakit]'");
                        ...
                        <tbody>
                            ...
                            <td><font face="Comic Sans MS, cursive" class="text-info text-left"><h5>
                                <?php
                                    $sg = $conn->query("select * from penyakit_gejala where kode_penyakit='$dpg[kode_penyakit]'");
                                    
                                    while($dg = mysqli_fetch_array($sg)){
                                        $sp1 = $conn->query("select * from gejala where id_gejala='$dg[id_gejala]'");
                                        $dp1 = mysqli_fetch_array($sp1);
                                        echo " $dp1[kode_gejala]";
