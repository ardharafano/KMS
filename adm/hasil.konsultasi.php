<?php
	...
	$jml_data = mysqli_num_rows($conn->query("select * from keterangan"));
	...
	if(isset($_GET['hal'])){
			...
			$s = $conn->query("select * from keterangan order by id_keterangan desc limit ".(($hal - 1) * $jml_per_halaman).", $jml_per_halaman");
		
	}else{
		$s = $conn->query("select * from keterangan order by id_keterangan desc limit 0, $jml_per_halaman")
				or die(mysqli_error());
	}
?>
			...
			<div>
                ...
                <div style="margin-top:10px">
                    ...
                    <thead>
                        ...
                        <td width="20%"><font face="Comic Sans MS, cursive" class="text-error text-center"><h5>Jenis Ayam</h5></font></td>
                        ...
                        <td width="20%"><font face="Comic Sans MS, cursive" class="text-error text-center"><h5>Nilai</h5></font></td>
                    ...
                    <tbody>
                        ...
                        <td><font face="Comic Sans MS, cursive" class="text-info text-left"><h5><?php echo "$jenis_ayam";?></h5></font></td>
                        ...
                        <td><font face="Comic Sans MS, cursive" class="text-info text-left"><h5>
							<?php
								$sh = $conn->query("select * from hasil_konsultasi where konsultasi='$id_konsultasi'");
								
								while($dh = mysqli_fetch_array($sh)){
									$sg = $conn->query("select * from gejala where id_gejala='$dh[id_gejala]'");
                        ...
                        <td><font face="Comic Sans MS, cursive" class="text-info text-center"><h5>
							<?php
								$sp=$conn->query("select * from penyakit where kode_penyakit='$kode_penyakit'");
								$dp=mysqli_fetch_array($sp);
								echo "$dp[nama_penyakit]";
							?>
                            </h5></font>
						</td>
						 <td><font face="Comic Sans MS, cursive" class="text-info text-center"><h5><?php echo "$nilai";?></h5></font></td>		
                        <td>
